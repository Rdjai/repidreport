...
const VolunteerDashboard: React.FC = () => {
    ...
    // Add this component to VolunteerDashboard.tsx


    // Function to fetch completed cases
    ...
    const handleCompleteAlert = async () => {
        ...
        try {
            ...
            localStorage.setItem('volunteerData', JSON.stringify(updatedVolunteer.data.data));

        } catch (error: any) {
            console.error('❌ Error completing alert:', error);
            alert(error.response?.data?.message || 'Error completing case');
        }
    };
                        ...
                        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[80vh] overflow-y-auto">
                            <div className="p-6">
                                <div className="flex justify-between items-center mb-6">
                                    <h2 className="text-2xl font-bold">Completed Cases History</h2>
                                    <button
                                            ...
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    ...
                                                    <p className="text-sm text-gray-600">
                                                        {caseItem.userInfo?.name || 'Anonymous User'}
                                                    </p>
                                                </div>
                                                <div className="text-sm text-gray-500">
                                                    {new Date(caseItem.resolvedAt || caseItem.createdAt).toLocaleDateString()}
                                                </div>
                                            </div>
                                            <div className="mt-2 text-sm">
                                                <p><strong>Location:</strong> {caseItem.location?.address || 'Not specified'}</p>
                                                <p><strong>Resolved at:</strong> {new Date(caseItem.resolvedAt).toLocaleTimeString()}</p>
                                            </div>
                                        </div>
                                    ))}
                        ...
                        <div className="grid grid-cols-3 gap-4">
                            ...
                            <div className="bg-white rounded-lg shadow p-4">
                                ...
                                <p className="text-2xl font-bold mt-2">
                                    {volunteerData.rating ? volunteerData.rating.toFixed(1) : '0.0'}/5
                                </p>
                            </div>
                        </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <button
                                            ...
                                            className="w-full bg-green-500 text-white py-2 rounded-lg font-semibold hover:bg-green-600 transition-colors flex items-center justify-center"
                                        >
